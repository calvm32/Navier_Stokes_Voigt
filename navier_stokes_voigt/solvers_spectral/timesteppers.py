...
def timestepper_compare_RK4(rhs1, rhs2, u_hat_0, f_hat_func, t0, T, dt):
    """
    Solve the ODE y' = f(t,y) on the interval [t0,T] with y(t0) = y0
    using the Runge-Kutta-4 approximation method,
    """
    ...
    for n in range(0, N - 1):
        ...
        k2 = rhs1(u_hat1[...,n] + dt/2 * k1, f_hat_func(t+dt/2))
        k3 = rhs1(u_hat1[...,n] + dt/2 * k2, f_hat_func(t+dt/2))
        k4 = rhs1(u_hat1[...,n] + dt * k3, f_hat_func(t+dt))

        u_hat1[...,n + 1] = u_hat1[...,n] + (dt/6)*(k1 + 2*k2 + 2*k3 + k4)

        k1 = rhs2(u_hat2[...,n], f_hat_func(t))
        k2 = rhs2(u_hat2[...,n] + dt/2 * k1, f_hat_func(t+dt/2))
        k3 = rhs2(u_hat2[...,n] + dt/2 * k2, f_hat_func(t+dt/2))
        k4 = rhs2(u_hat2[...,n] + dt * k3, f_hat_func(t+dt))

        u_hat2[...,n + 1] = u_hat2[...,n] + (dt/6)*(k1 + 2*k2 + 2*k3 + k4)
...
def timestepper_intfactor_RK4(rhs, u_hat_0, f_hat_func, t0, T, dt, L_hat=0):
    """
    Solve the ODE y' = f(t,y) on the interval [t0,T] with y(t0) = y0
    using the Runge-Kutta-4 approximation method,
    ...
    # integrating factor
    Efull = np.exp(L_hat*dt)
    Ehalf = np.exp(L_hat*dt/2)
    ...
    for n in range(0, N - 1):
        ...
        #k4 = rhs(Efull*(u_hat[...,n]) + Ehalf* (dt * k3), f_hat_func(t+dt))

        u_hat[...,n + 1] = Efull*u_hat[...,n] + (dt/6)*(Efull*k1 + Ehalf*2*(k2 + k3) + k4)

    return u_hat, times

def timestepper_intfactor_compare_RK4(rhs1, rhs2, u_hat_0, f_hat_func, t0, T, dt, L_hat=0):
    """
    Solve the ODE y' = f(t,y) on the interval [t0,T] with y(t0) = y0
    using the Runge-Kutta-4 approximation method,
    specifically with integrating factor of e^power applied
