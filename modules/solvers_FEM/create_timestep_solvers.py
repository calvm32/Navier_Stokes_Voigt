...
def create_timestep_solver_CN(get_data, Z, dx , ds, u_old, u, make_weak_form, is_mixed, theta, gamma, Re,
    ...
    if is_mixed:
        ...
        if alpha == None:
            a, L = make_weak_form(
                idt, f, f_old, g, g_old, u_old, dx, ds, theta, gamma, nu
            )(u, TestFunction(Z))
        else: 
            a, L = make_weak_form(
                idt, f, f_old, g, g_old, u_old, dx, ds, theta, gamma, nu, alpha
            )(u, TestFunction(Z))
        F = a - L
        ...
        a, L = make_weak_form(
            idt, f, f_old, g, g_old, u_old, dx, ds, theta, gamma
...
def create_timestep_solver_BDF2(get_data, Z, dx , ds, u_older, u_old, u, make_weak_form, is_mixed, gamma, Re,
                           alpha, bcs=None, nullspace=None, solver_parameters=None, appctx=None, Umax=1, char_length=1):
    """
    ...
    if is_mixed:
        ...
        if alpha == None:
            a, L = make_weak_form(
                idt, f, f_old, g, g_old, u_older, u_old, dx, ds, gamma, nu
            )(u, TestFunction(Z))
        else: 
            a, L = make_weak_form(
                idt, f, f_old, g, g_old, u_older, u_old, dx, ds, gamma, nu, alpha
            )(u, TestFunction(Z))
        F = a - L
        problem_var = NonlinearVariationalProblem(F, u, bcs=bcs)
        solver = NonlinearVariationalSolver(
        ...
        a, L = make_weak_form(
            idt, f, f_old, g, g_old, u_older, u_old, dx, ds, gamma
        )(u_trial, TestFunction(Z))
        problem_var = LinearVariationalProblem(a, L, u, bcs)
        solver = LinearVariationalSolver(
            problem_var,
